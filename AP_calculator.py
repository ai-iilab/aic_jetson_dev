...
def AP_evaluation(gt, pred):
    ...
    for img in gt_data['annotations']:
        ...
        gt_len += len(img['objects'])

    # predictions
    with open(pred, 'r') as json_file:
        pred_data = json.load(json_file)

    param_num = int(pred_data['param_num'])
    inference_time = int(pred_data['inference_time'])

    img_annots = pred_data['annotations']

    assert len(img_annots) != 0, 'Empty annotations'
    ...
    for img in img_annots:
        ...
        if len(img['objects']) == 0: # exception handling
            continue

        # sort bboxes by confidence score
        sort_boxes = sorted(img['objects'], key=itemgetter('confidence_score'), reverse=True)
        ...
        for obj in sort_boxes:
            try:
                ...
                if len(obj['position']) != 4: # exception handling
                    raise Exception('position length')
    ...
    assert len(predictions) != 0, 'Empty list of predictions'
